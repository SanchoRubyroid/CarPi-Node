...
class VehicleManager{
  ...
  removeVehicle(socket){
    let vehicleToRemove = getVehicleBySocketID(socket.id)
  ...
  captureVehicle(vehicleName, controlSocket){
    let vehicle = getVehicleByName(vehicleName)
  ...
  releaseVehicle(controlSocket){
    let vehicle = getVehicleByControlSocketID(controlSocket.id)
  ...
  getControlSocket(vehicleName){
    let vehicle = getVehicleByName(vehicleName.toString().replace(Vehicle.namePattern, ''))
    return vehicle.getControlSocket()
  }

  setVehicleName(data, socket){
    let vehicleName = data.toString().replace(Vehicle.namePattern, '')
    console.log('Vehicle name: ' + vehicleName)
    let vehicle = getVehicleBySocketID(socket.id)
