...
class ConnectionUtils{
  ...
  initializeBrowserControlConnection() {
    this.io.of('/control').on('connection', (socket) => {
      ...
      socket.on('video-settings', (data) => {
        let streamProcessor = new StreamProcessor(this.config)
        this.vehicleManager.setVideoQuality(socket, data.videoQuality, streamProcessor)
      })
    })
    ...
    let vehiclesServer = net.createServer((socket) => {
      ...
      socket.on('data', (data) => {
        if(data == 'pong'){
          ...
          // TODO TCP does not guarantee any boundaries, so sometimes 'pong'
          // is received together with settings :(
